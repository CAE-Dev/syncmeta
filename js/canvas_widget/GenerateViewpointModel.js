        ...
        function GenerateViewpointModel(viewpointModel){
            ...
            for(var edge_key in viewpointModel.edges){
                if(viewpointModel.edges.hasOwnProperty(edge_key)){
                    ...
                    EntityManager.createEdgeFromJSON(vpEdge.type, edge_key, vpEdge.source, vpEdge.target, vpEdge);
                }
            }
            ...
            function getConcreteObjectNodeTypes(node, visitedNodes) {
                ...
                if ((node instanceof ObjectNode || node instanceof  ViewObjectNode) && classTypes.indexOf(type) === -1) {
                ...
                for (edgeId in ingoingEdges) {
                    if (ingoingEdges.hasOwnProperty(edgeId)) {
                        ...
                        if (edge instanceof GeneralisationEdge && source instanceof ObjectNode ||
                            ...
                            edge instanceof GeneralisationEdge && source instanceof ViewObjectNode) {
                        ...
                        for (edgeId in edges) {
                            if (edges.hasOwnProperty(edgeId)) {
                                ...
                                if ((edge instanceof BiDirAssociationEdge &&
                                    ...
                                    target === node && (neighbor = source)instanceof ViewObjectNode ||
                                    source === node && (neighbor = target)instanceof ObjectNode ||
                                    source === node && (neighbor = target)instanceof ViewObjectNode))) {
                                ...
                                } else if (edge instanceof UniDirAssociationEdge && source === node && (target instanceof ObjectNode || target instanceof ViewObjectNode)) {

                                    targetTypes = targetTypes.concat(getConcreteObjectNodeTypes(target));

                                } else if (edge instanceof UniDirAssociationEdge && target === node && (source instanceof ObjectNode || source instanceof ViewObjectNode)) {
                                    ...
                                    for (groupEdgeId in groupEdges) {
                                        if (groupEdges.hasOwnProperty(groupEdgeId)) {
                                            ...
                                            } else if (groupEdge instanceof UniDirAssociationEdge && groupSource === neighbor && (groupTarget instanceof ObjectNode || groupTarget instanceof ViewObjectNode)) {
